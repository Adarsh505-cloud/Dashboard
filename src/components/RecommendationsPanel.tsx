...
const RecommendationsPanel: React.FC<RecommendationsPanelProps> = ({ data }) => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'oversized': return Server;
      case 'unused': return Database;
      case 'optimization': return HardDrive;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">
                {data.filter(r => r.type === 'idle').length}
              </div>
              <div className="text-gray-600">Idle Resources</div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-xl font-semibold text-gray-900">Cost Optimization Recommendations</h3>
          <p className="text-gray-600 mt-1">Resources idle for 7+ days and optimization opportunities</p>
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {data.map((recommendation) => {
            ...
            return (
              <div key={recommendation.id} className="p-6 hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getSeverityColor(recommendation.severity)}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        <span>Last activity: {recommendation.lastActivity}</span>
                      </div>
