...
const TopSpendingResources: React.FC<TopSpendingResourcesProps> = ({ topSpendingResources }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Top Spending Resources (Last 30 Days)</h3>
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-auto">
          <thead className="bg-gray-50 dark:bg-gray-900">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Resource Type</th>
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {visibleResources.map((resource, index) => {
              ...
              return (
                <tr key={keyId} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg">{getResourceIcon(serviceName)}</div>
                      <div className="ml-4"><div className="text-sm font-medium text-gray-900 dark:text-gray-100">{serviceName}</div></div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap max-w-xs">
                    <div className="flex items-center gap-2">
                      <div className={`text-sm truncate ${isCopyDisabled ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'}`} title={rawId || displayId} style={{ maxWidth: '12rem' }}>{displayId}</div>
                      <button onClick={() => !isCopyDisabled && navigator.clipboard?.writeText(rawId || displayId)} title={isCopyDisabled ? 'No ID to copy' : `Copy full ID: ${rawId || displayId}`} className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50" disabled={isCopyDisabled}><Copy className="w-4 h-4 text-gray-500 dark:text-gray-400" /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400" title={resource.region || ''}>{displayRegion}</td>
                  <td className="px-6 py-4 whitespace-nowrap"><span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300">{resource.resource_type}</span></td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900 dark:text-gray-100">${Number(resource.total_cost || 0).toFixed(2)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      {topSpendingResources.length > 5 && (<div className="mt-4 text-center"><button onClick={handleToggle} className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">{hasMore ? 'Show More' : 'Show Less'}</button></div>)}
    </div>
  );
};
