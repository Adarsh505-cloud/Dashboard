...
const ResourceChart: React.FC<ResourceChartProps> = ({ data, dailyCostData, weeklyCostData, topSpendingResources, isExporting }) => {
      ...
      return (
        <div className="flex items-center justify-center h-full">
          <div className="text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No Resource Data Available</h3>
            <p className="text-gray-500 dark:text-gray-400">No resource cost data found for your AWS account</p>
          </div>
        </div>
      );
    }
  ...
  if (validResources.length === 0) {
    return (
      <div className="space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-12 text-center">
          <Activity className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
          <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">No Resource Data Available</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-4">No resource cost data found for your AWS account. This could be because:</p>
          <div className="text-left max-w-md mx-auto"><ul className="list-disc list-inside text-gray-500 dark:text-gray-400 space-y-1"><li>No resources are currently running</li><li>Resources don't have cost data yet</li><li>Cost data is still being processed by AWS</li><li>Resources are in different regions not covered</li></ul></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-3 sm:p-4 lg:p-6">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div>
            <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 dark:text-gray-100">Cost Analysis Controls</h3>
            <p className="text-gray-500 dark:text-gray-400">Adjust the time range and chart type for the data below</p>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 rounded-xl p-1">
                {(['daily', 'weekly', 'monthly'] as const).map((range) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-2 ${timeRange === range ? 'bg-white dark:bg-gray-800 text-blue-600 shadow-sm dark:shadow-gray-900/20' : 'text-gray-600 dark:text-gray-400 hover:text-blue-600'}`}
                  >
             ...
             <div className="flex items-center gap-3">
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Chart Type</span>
              <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 rounded-xl p-1">
                {([
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-2 ${chartType === type ? 'bg-white dark:bg-gray-800 text-blue-600 shadow-sm dark:shadow-gray-900/20' : 'text-gray-600 dark:text-gray-400 hover:text-blue-600'}`}
                  >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-3 sm:p-4 lg:p-6">
        <div className="flex items-center justify-between mb-6">
          <div><h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 dark:text-gray-100">Resource Cost Trends - {timeRange.charAt(0).toUpperCase() + timeRange.slice(1)} View</h3><p className="text-gray-500 dark:text-gray-400">{chartType === 'doughnut' ? 'Current cost distribution across resource types' : `Top 8 spending services for the selected period`}</p></div>
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400"><Globe className="w-4 h-4" /><span>All Regions</span></div>
        </div>
        <div className={`${chartType === 'doughnut' ? 'h-48 sm:h-64 md:h-80' : 'h-48 sm:h-64 md:h-80'}`}>{renderChart()}</div>
        {chartType !== 'doughnut' && validResources.length > 0 && (<div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 lg:gap-6"><div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800"><div className="flex items-center gap-2 mb-2"><TrendingUp className="w-5 h-5 text-blue-600" /><span className="font-medium text-blue-800 dark:text-blue-300">{timeRange === 'daily' ? 'Daily Range' : 'Cost Range'}</span></div><div className="text-sm text-blue-700 dark:text-blue-400">{timeRange === 'daily' && costRange.max > 0 ? `$${costRange.min.toFixed(2)} - $${costRange.max.toFixed(2)} per day` : 'Real AWS cost data'}</div></div><div className="p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800"><div className="flex items-center gap-2 mb-2"><DollarSign className="w-5 h-5 text-green-600" /><span className="font-medium text-green-800 dark:text-green-300">Highest Cost</span></div><div className="text-sm text-green-700 dark:text-green-300">{(() => { const highestCost = validResources.reduce((max, resource) => resource.cost > max.cost ? resource : max); return `${highestCost.type}: $${highestCost.cost.toLocaleString()}/month`; })()}</div></div><div className="p-4 bg-purple-50 dark:bg-purple-950 rounded-lg border border-purple-200 dark:border-purple-800"><div className="flex items-center gap-2 mb-2"><Activity className="w-5 h-5 text-purple-600" /><span className="font-medium text-purple-800 dark:text-purple-300">Most Resources</span></div><div className="text-sm text-purple-700 dark:text-purple-300">{(() => { const mostResources = validResources.reduce((max, resource) => resource.count > max.count ? resource : max); return `${mostResources.type}: ${mostResources.count} resources`; })()}</div></div></div>)}
        <div className="mt-4 p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg"><div className="text-sm text-green-800 dark:text-green-300"><strong>✅ Real AWS Data:</strong>{timeRange === 'daily' && dailyCostData ? ` Daily costs fetched directly from AWS Cost Explorer (${dailyCostData.length} days of actual billing data from ${dailyCostData.length > 0 ? new Date(dailyCostData[0].TimePeriod.Start).toLocaleDateString() : 'N/A'}).` : timeRange === 'weekly' && weeklyCostData ? ` Weekly costs aggregated from AWS Cost Explorer (${weeklyCostData.length} weeks of actual billing data from ${weeklyCostData.length > 0 ? new Date(weeklyCostData[0].TimePeriod.Start).toLocaleDateString() : 'N/A'}).` : ' Monthly cost trends from your AWS Cost Explorer data.'}</div></div>
      </div>
      <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-3 sm:p-4 lg:p-6">
        <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 dark:text-gray-100 mb-6">Resource Type Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          {validResources.map((resource, index) => {
            ...
            return (
              <div key={resource.type} className={`p-3 sm:p-4 lg:p-6 rounded-xl sm:rounded-2xl border-2 transition-all duration-200 cursor-pointer ${selectedResource === resource.type ? 'border-blue-500 bg-blue-50 dark:bg-blue-950' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 hover:shadow-md dark:hover:shadow-gray-900/20'}`} onClick={() => setSelectedResource(selectedResource === resource.type ? null : resource.type)}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div><h4 className="font-semibold text-gray-900 dark:text-gray-100">{resource.type.replace('Amazon ', '').replace(' Service', '')}</h4><p className="text-sm text-gray-500 dark:text-gray-400">{resource.count} resources</p></div>
                  </div>
                  <div className="text-right">
                    <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">${resource.cost.toLocaleString()}</div>
                ...
                {selectedResource === resource.type && (<div className="mt-4 pt-4 border-t border-blue-200 dark:border-blue-800"><div className="grid grid-cols-2 gap-4 text-sm"><div><span className="text-gray-500 dark:text-gray-400">Avg. Cost per Resource:</span><div className="font-medium">${(resource.cost / resource.count).toFixed(2)}</div></div><div><span className="text-gray-500 dark:text-gray-400">Real Daily Avg:</span><div className="font-medium">{timeRange === 'daily' && costRange.max > 0 ? `$${((costRange.min + costRange.max) / 2).toFixed(2)}` : 'N/A'}</div></div></div></div>)}
              </div>
            );
          })}
        </div>
