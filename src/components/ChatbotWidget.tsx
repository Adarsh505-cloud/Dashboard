...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ accountId, accountType }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6" style={{ zIndex: 9999 }}>
      {/* Chat Window */}
      {isOpen && (
        <div className="bg-white border border-gray-200 rounded-2xl shadow-2xl w-80 sm:w-96 h-[560px] flex flex-col mb-4 overflow-hidden transition-all duration-300 transform origin-bottom-right">
              ...
              <div className="flex flex-wrap gap-1.5">
                {QUICK_QUERIES.map((q) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="text-xs px-2.5 py-1.5 rounded-full border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:border-blue-400 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap font-medium"
                  >
                    {q.label}
                  </button>
                ))}
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-gray-100 flex gap-2 flex-shrink-0">
            <input
              ...
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              disabled={isLoading}
            />
            <button
              type="submit"
